Build a section of a simple video game web where we should
be able to add users and display users, along with the number of games
he/she has played. 

Create an app that lets you add a user's first name, last name, and username.
When the user is added, the number of games that he/she has played is defaulted to 0. Each username has to be unique, so we cannot add multiple users with the
same username. When someone clicks Add but the username already exists, 
the app should not allow for a duplicate user to be added and should show an
